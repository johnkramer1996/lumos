...
const CoursesLessonsGroup = ({ id, name, lessons, currentLesson, courseId, rolesId }) => {
   let index = 0
            ...
            {lessons.map(({ id, name, progress }, indexLesson) => {
               ...
               return (
                  <div key={id || indexLesson}>
                     {currentLesson.id === id ? (
                        <Link to={getURL.cabinetCoursesLesson({ courseId, lessonId: id }, rolesId)} className='lkp-course__current'>
                        ...
                        <Link to={getURL.cabinetCoursesLesson({ courseId, lessonId: id }, rolesId)} className='lkp-course__item'>
                           <i></i>
